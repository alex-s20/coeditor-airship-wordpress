...
add_action(
	...
	function () {
		...
		register_setting(
			'coeditor_airship_settings',
			'coeditor_airship_custom_worker_js',
			[
...
add_action(
	'admin_post_coeditor_airship_delete_worker',
	function() {
...
add_action(
	...
	function () {
		wp_enqueue_style(
			'coeditor-airship',
			COEDITOR_AIRSHIP_ASSETS . 'css/index.css',
			[],
			COEDITOR_AIRSHIP_VERSION
		);
		wp_enqueue_script(
			'coeditor-airship',
			COEDITOR_AIRSHIP_ASSETS . 'js/index.js',
			[ 'jquery' ], // jQuery optional, just needs to be any dependency for timing
			COEDITOR_AIRSHIP_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			[
				...
				'nonce'   => wp_create_nonce( 'coeditor_airship_nonce' ),
			]
		);
	}
);
